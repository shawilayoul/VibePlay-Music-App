    ...
    useEffect(() => {
        const getUserFavoriteTracks = async () => {
            ...
            try {
                ...
                const likedTrackIds = response.data.favorites.playlistTrackFavorites.map((t: any) => t?.id);
    ...
    const toggleTrackLike = async (trackId: string, action: string) => {
        ...
        try {
            const response = await axios.post(
                `https://musicserver-uluy.onrender.com/favorites/playlist-tracks/${action}`,
                { trackId },
                {
                    headers: {
            ...
            if (action === 'like') {
                ...
                setFavorites((prevLikedTracks) => [...prevLikedTracks, response.data.track]); // Add the liked track
